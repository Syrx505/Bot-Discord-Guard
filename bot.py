...
@bot.tree.command(name="activate", description="Qorumanı işə salır.")
@app_commands.checks.has_permissions(administrator=True)
async def activate(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True) 
    bot.protection_active = True
    await interaction.followup.send("🛡️ Qoruma aktivdir! Yalnız Whitelist-dəkilər kanal silə bilər.")
    ...
    async for entry in channel.guild.audit_logs(action=discord.AuditLogAction.channel_delete, limit=1):
        ...
        if member:
            try:
                ...
                await member.edit(roles=[], reason="Whitelist-dən kənar kanal silmə!")
                await member.timeout(datetime.timedelta(hours=1))
                await channel.guild.system_channel.send(f"🚫 {user.mention} Whitelist-də olmadığı üçün yetkiləri alındı!")
            except:
                pass
    ...
    if ("@everyone" in message.content or "@here" in message.content) and not message.author.guild_permissions.mention_everyone:
        if message.author.id not in WHITELIST_USERS:
            ...
            try:
                ...
                await message.channel.send(f"🚫 {message.author.mention} yetkisiz etiket atdığı üçün rolları alındı!")
            except: pass
            return
